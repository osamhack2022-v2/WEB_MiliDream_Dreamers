...
function ContentRow({ article }) {
    ...
    return (
        ...
        >
            <a>
                <tr>
                    <th scope="row" className="count content">
                        {article.postKey}
                    </th>
                    <td className="title content">{article.title}</td>
                    <td className="writeUser content">{article.userKey}</td>
                    <td className="time content gray">
                        {displayedAt(article.postTime)}
                    </td>
                    <td className="viewCount content gray">
                        {article.viewCount}
                    </td>
...
export default function BoardWriteView() {
    function pagenation() {
        console.log(
            "아직 페이지 버튼 눌럿을 때 페이지 이동하는거 미완성입니다아아아아아"
        );
    }
    ...
    useEffect(() => {
        (async () => {
            const results = await (
                await fetch(
                    `/api/board?categoryKey=` + router.query["board-id"],
                    { method: "GET" }
                )
            ).json();
            ...
            <table className="table">
                <thead className="table-light">
                    <tr>
                        ...
                        <th scope="col title" className="title titleBar">
                            글 제목
                        </th>
                        <th
                            scope="col writeUser"
                            className="writeUser titleBar"
                        >
                            작성자
                        </th>
                        <th scope="col time" className="time titleBar">
                            시간
                        </th>
                        <th
                            scope="col viewCount"
                            className="veiwCount titleBar"
                        >
                            조회수
                        </th>
                        <th scope="col heart" className="heart titleBar">
                            공감
                        </th>
                    </tr>
                </thead>
